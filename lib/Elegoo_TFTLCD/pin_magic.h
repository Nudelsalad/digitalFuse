 ...
 // Arduino Mega, ADK, etc.
  ...
  #define setReadDirInline()  {                                   \
    ...
    PORTB |= ((d & 0xC) << 2);\
  	...
  	PORTG |= (d & 0x10) << 1;\ 
    WR_STROBE; }
  #define read8inline(result) { RD_ACTIVE; DELAY7; result = (PINH & 0x60) >> 5;result |= (PINH & 0x18) << 3;result |= (PINE & 0x8) << 2;result |= (PINB & 0x30) >> 2;result |= (PING & 0x20) >> 1;RD_IDLE;}
  #define setWriteDirInline() { DDRH |= 0x78;DDRE |= 0x8;DDRB |= 0x30;DDRG |= 0x20; }
  #define setReadDirInline()  { DDRH &= ~0x78;DDRE &= ~0x8;DDRB |=0x30;DDRG &= ~(0x20); }
